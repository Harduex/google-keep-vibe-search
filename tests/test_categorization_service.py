...
def test_sanitize_tag_name():
    # JSON input
    assert CategorizationService._sanitize_tag_name('{"tag": "Home Renovation"}') == "Home Renovation"
    
    # Quoted input
    assert CategorizationService._sanitize_tag_name('"Travel Plans"') == "Travel Plans"
    
    # 4+ word input truncates to 3
    assert CategorizationService._sanitize_tag_name("My Awesome Travel Plans Today") == "My Awesome Travel"
...
def test_apply_merge_map():
    ...
    vocab.add(Label(name="Gym", seed_note_ids=["1", "2"], sample_notes=[{"title": "A"}], confidence=0.8, source="cluster", is_anchor=False))
    vocab.add(Label(name="Workout", seed_note_ids=["3"], sample_notes=[{"title": "B"}], confidence=0.6, source="cluster", is_anchor=False))
    vocab.add(Label(name="Recipes", seed_note_ids=["4"], sample_notes=[{"title": "C"}], confidence=0.9, source="cluster", is_anchor=False))
...
def test_harvest_title_prefixes():
    ...
    original = settings.prefix_min_count
    settings.prefix_min_count = 2
    ...
    try:
        ...
        settings.prefix_min_count = original
...
def test_tf_idf_keyword_extraction():
    ...
    keywords_by_cluster = CategorizationService._get_hint_keywords([cluster1, cluster2], max_words=2)
    ...
    assert any(w in ["baking", "recipe", "flour", "sugar", "cooking"] for w in keywords_by_cluster[1])
