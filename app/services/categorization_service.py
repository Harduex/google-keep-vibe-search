...
class CategorizationService:
    ...
    def _harvest_title_prefixes(notes: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        return {k: v for k, v in counts.items() if v >= settings.prefix_min_count}
    ...
    def _apply_merge_map(vocab: LabelVocabulary, merge_map: Dict[str, Any]) -> None:
        ...
        for merge in merges:
            ...
            if total_count > 0:
                weighted_conf = sum(c.confidence * len(c.seed_note_ids) for c in constituents) / total_count
            else:
                weighted_conf = 0.0
    ...
    @staticmethod
    def _get_hint_keywords(clusters: List[List[Dict[str, Any]]], max_words: int = 5) -> List[List[str]]:
        if not clusters:
            return []
        ...
        try:
            ...
            for cluster_notes in clusters:
                ...
                bg_stops = {
                    "това", "като", "за", "на", "не", "да", "се",
                    "от", "със", "или", "има", "какво", "къде", "кога", "този", "тази",
                    "тези", "ако", "без", "беше", "всички", "дали", "до", "докато",
                    "защо", "които", "който", "където", "няма", "още", "само", "след",
                    "така", "че", "ще"
                }
                generic_stops = {"https", "http", "com", "www", "use", "using", "used", "just", "like", "get"}
        ...
        documents = []
        for cluster_notes in clusters:
            doc_text = " ".join(f"{n.get('title', '')} {n.get('content', '')}" for n in cluster_notes)
    ...
    @staticmethod
    def _get_mmr_sample(embeddings: np.ndarray, centroid: np.ndarray, num_samples: int = 10, lambda_param: float = 0.5) -> List[int]:
    ...
    async def _get_llm_tag_name(self, notes_text: str, keywords: str, neighbor_keywords: str) -> str:
        sys_prompt = TAG_NAMING_SYSTEM_PROMPT
        user_prompt = TAG_NAMING_USER_PROMPT.format(notes_text=notes_text, keywords=keywords, neighbor_keywords=neighbor_keywords)
        ...
        for attempt in range(3):
            try:
                ...
                if not raw or not raw.strip():
                    print(f"Empty or whitespace response on attempt {attempt + 1}, raw={repr(raw)}. Retrying...")
                    if attempt < 2:
    ...
    async def categorize(self, granularity: str = "broad") -> AsyncGenerator[bytes, None]:
        try:
            ...
            umap_components, umap_neighbors, min_cluster_size, min_samples = self._get_cluster_sizing(granularity, n)
            ...
            if prefixes:
                try:
                    ...
                    for target, info in cat_map.items():
                        ...
                        if matching_indices:
                            sample = [self._truncate_note(notes[idx]) for idx in matching_indices[:5]]
                            vocab.add(Label(
                                name=tag_name,
            ...
            for cluster_idx, (cluster_label, member_indices) in enumerate(cluster_items):
                ...
                representative_indices = self._get_mmr_sample(cluster_embeddings, centroid, num_samples=10)
                ...
                keywords_list = cluster_keywords_list[cluster_idx] if cluster_idx < len(cluster_keywords_list) else []
                ...
                if len(cluster_centroids) > 1:
                    ...
                    neighbor_keywords_list = cluster_keywords_list[nearest_idx][:3] if nearest_idx < len(cluster_keywords_list) else []
            ...
            async def _name_labels_async():
                try:
                    ...
                    for i, (lbl, n_text, kw_str, neighbor_kw) in enumerate(llm_tasks):
                        ...
                        denylist = {"misc", "miscellaneous", "various", "general", "other", "notes", "undefined", "unknown", "uncategorized"}
                        if real_name and real_name.lower() not in denylist:
                            lbl.name = real_name
                        else:
                            if not real_name:
                                lbl.name = " ".join(kw_str.split(", ")[:2]).title() if kw_str else "Misc"
                            else:
                                lbl.name = "DROP_ME"
                    ...
                    await queue.put(self._line({"type": "progress", "stage": "naming", "message": "Consolidating tags...", "progress": 0.92}))
                    
                    try:
                        valid_labels = [lbl for lbl in vocab.labels if lbl.name != "Uncategorized" and lbl.prototype_vector is not None]
                        merged_into = {lbl.name: lbl.name for lbl in valid_labels}
                        
                        def find_root(x):
                            if merged_into[x] == x: return x
                            merged_into[x] = find_root(merged_into[x])
                            return merged_into[x]
                            
                        def union_roots(x, y):
                            rx, ry = find_root(x), find_root(y)
                            if rx != ry: merged_into[ry] = rx
                        ...
                        remaining_labels = [lbl for lbl in vocab.labels if lbl.name != "Uncategorized"]
                        ...
                        if active_borderline or len(remaining_labels) > settings.max_tags:
                            ...
                            prompt += "You must reduce the list to at most " + str(settings.max_tags) + " general, human-readable tags of 1-2 English words each.\n\n"
                            if anchor_tags:
                                prompt += "The following tags MUST NOT be merged away, they are anchors: " + ", ".join(anchor_tags) + "\n\n"
                                
                            prompt += "Borderline Pairs to Consider Merging:\n"
                            for a, b in active_borderline:
                                prompt += f"- Pair: '{find_root(a.name)}' and '{find_root(b.name)}'\n"
                            ...
                            prompt += "\nAll Current Tags:\n"
                            pairs = [{"tag": lbl.name, "count": len(lbl.seed_note_ids)} for lbl in remaining_labels]
                            ...
                            remaining_labels = [lbl for lbl in vocab.labels if lbl.name != "Uncategorized"]
                            while len(remaining_labels) > settings.max_tags:
                                # Find closest pair
                                best_sim = -1
                                best_pair = None
                                for i in range(len(remaining_labels)):
                                    for j in range(i+1, len(remaining_labels)):
                                        ...
                                        if sim > best_sim:
                                            best_sim = sim
                                            best_pair = (remaining_labels[i].name, remaining_labels[j].name)
                                if best_pair:
                                    self._apply_merge_map(vocab, {"merges": [{"into": best_pair[0], "from": [best_pair[1]]}]})
                                    remaining_labels = [lbl for lbl in vocab.labels if lbl.name != "Uncategorized"]
                                else:
                                    break
                    ...
                    await queue.put(self._line({"type": "progress", "stage": "assigning", "message": "Assigning notes to final labels...", "progress": 0.95}))
                    ...
                    await queue.put(self._line({"type": "label_updates", "proposals": vocab.to_proposals()}))
    ...
    def _build_prototype_vectors(self, vocab: LabelVocabulary, embeddings: np.ndarray, notes: List[Dict[str, Any]], note_indices: List[int]) -> None:
    ...
    def _assign_labels_via_embeddings(self, vocab: LabelVocabulary, embeddings: np.ndarray, notes: List[Dict[str, Any]], note_indices: List[int]) -> None:
        ...
        norm_embeddings = np.divide(embeddings, emb_norms, out=np.zeros_like(embeddings), where=emb_norms!=0)
        ...
        for j, lbl in enumerate(valid_labels):
            ...
            sims = similarities[:, j]
            assigned_indices = [i for i, idx in enumerate(note_indices) if notes[idx]["id"] in lbl.seed_note_ids]
        ...
        if uncat_label and uncat_label.seed_note_ids:
            uncat_indices = [i for i, idx in enumerate(note_indices) if notes[idx]["id"] in uncat_label.seed_note_ids]
            sample = [self._truncate_note(notes[note_indices[i]]) for i in uncat_indices[:5]]
            uncat_label.sample_notes = sample
            
        vocab.labels = [lbl for lbl in vocab.labels if lbl.seed_note_ids or lbl.name == "Uncategorized"]
